...
class mobile_core {
	...
	//解析客户端表情
	static function smiley($message) {
		preg_match_all('/<img[^>]*\>/', $message, $match);
		if($match) {
			foreach($match[0] as $img) {
				preg_match('/<img src="(.*?)"/', $img, $m);
				$im = $m[1];
				$dir = 'static/image/smiley/';
				if(strstr($im, $dir)) {
					$str = strrev($im);
					$url = substr($str, 0, strpos($str, '/'));
					$url = strrev($url);
					$file = substr($str, strpos($str, '/') + 1);
					$file = substr($file, 0, strpos($file, '/'));
					$file = strrev($file);
				} else {
					return $message;
				}
				$type = DB::fetch_first("SELECT typeid FROM " . DB::table('forum_imagetype') . " WHERE directory = '" . $file . "'");
				$smiley = DB::fetch_first("SELECT id,displayorder, code FROM " . DB::table('common_smiley') . " WHERE typeid = " . $type['typeid'] . " AND url = '" . $url . "'");

				// 1:ios 2:android
				if($_GET['mapifrom'] == 'ios') {
					$imgstr = sprintf('%02d_%02d', $type['typeid'], $smiley['displayorder']);
					$code = '#[' . $imgstr . ']';
				} elseif($_GET['mapifrom'] == 'android') {
					$code = '<img src="{:' . $type['typeid'] . '_' . $smiley['id'] . ':}" />';
				}
	...
	function thumb($aid, $nocache = 0, $w = 140, $h = 140, $type = '') {
		global $_G;
		$key = dsign($aid.'|'.$w.'|'.$h);
		return $_G['siteurl'].'api/mobile/?module=forumimage&aid='.$aid.'&size='.$w.'x'.$h.'&key='.rawurlencode($key).($nocache ? '&nocache=yes' : '').($type ? '&type='.$type : '').'&version=5';
	}
}
