<template>
	<view
		...
		:class="{ 'bar-line': opacity > 0.85 && splitLine, 'navbar-fixed': isFixed, backdrop__filter: backdropFilter && dropDownOpacity > 0 }"
		:style="{ height: height + 'px', backgroundColor: `rgba(${background},${opacity})`, opacity: dropDownOpacity, zIndex: isFixed ? zIndex : 'auto' }"
	>
		<view class="status-bar" :style="{ height: statusBarHeight + 'px' }" v-if="isImmersive"></view>
		<view
...
export default {
	...
	data() {
		return {
			...
			background: 'var(--base-rgb)', //header背景色
			dropDownOpacity: 1
		};
	},
	created() {
