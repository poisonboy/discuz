<template>
	<view class="container">
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :up="upOption">
			<nui-list-stick :sticklist="toplist"></nui-list-stick>
		...
		methods: {
			...
			upCallback(page) {
				...
				getCategoryRequest.then(dataList => {
					this.toplist = dataList.data.Variables.top_threadlist;
					//联网成功的回调,隐藏下拉刷新和上拉加载的状态; 
					this.mescroll.endSuccess(dataList.data.Variables.forum_threadlist.length);
					// 过滤掉置顶帖，只保留普通帖子

					//如果是第一页需手动制空列表
					if (page.num == 1) this.dataList = [];
					//追加新数据
					this.dataList = this.dataList.concat(dataList.data.Variables.forum_threadlist);
				}).catch(() => {
					//联网失败, 结束加载
