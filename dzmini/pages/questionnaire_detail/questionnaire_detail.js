...
Page({
  ...
  onShareAppMessage: function(res) {
    ...
    var title = self.data.thread.subject ? self.data.thread.subject : self.data.nickname + "邀请你参与问卷调查，快快加入讨论吧"
  ...
  submitPoll() {
    ...
    for (let i = 0; i < self.data.pollanswers.length; i++) {
      var keyStr = "pollanswers[" + i + "]"
      postData[keyStr] = self.data.pollanswers[i]
    }

    app.apimanager.postRequest(urlStr, postData).then(res => {
  ...
  lookjoin() {
    wx.navigateTo({
      url: '../vote_list/vote_list?tid=' + self.data.tid,
    })
  },
})